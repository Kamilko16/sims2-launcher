Public Class MyVerticalScrollBarCB
    ...
    Public Sub CalculateHeight(ByVal items As Integer)
        CreateScrollBar(Math.Max(12, Me.Height - Math.Max(0, (15 * (items - (CType(Me.Parent, MyListBoxCB).Radio_External_Container.Height / 15)) / items * Math.Sqrt(CType(Me.Parent, MyListBoxCB).Items)))))
        ScrollButton.Height = Math.Max(12, Me.Height - Math.Max(0, (15 * (items - (CType(Me.Parent, MyListBoxCB).Radio_External_Container.Height / 15)) / items * Math.Sqrt(CType(Me.Parent, MyListBoxCB).Items))))
    ...
    Private Sub ScrollButton_MouseMove(sender As Object, e As MouseEventArgs) Handles ScrollButton.MouseMove
        If IsMouseDown Then
            ...
            Dim test As Double = (Me.Height - ScrollButton.Height) / ((CType(Me.Parent, MyListBoxCB).Items - (CType(Me.Parent, MyListBoxCB).Radio_External_Container.Height / 15)))
            ...
            If test > 0 Then
                ...
                CType(Me.Parent, MyListBoxCB).Radio_Internal_Container.Top = Math.Max(Math.Round((-ScrollButton.Top * Math.Sqrt(CType(Me.Parent, MyListBoxCB).Items)) / 15) * 15, -CType(Me.Parent, MyListBoxCB).Radio_Internal_Container.Height + CType(Me.Parent, MyListBoxCB).Radio_External_Container.Height)
